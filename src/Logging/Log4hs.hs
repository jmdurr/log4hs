...
defLoggerCtx =
  LogContext
    { ctxAppenders = [("console", consoleAppender SYSTEM_OUT (patternLayout (pack "%maxLen{%d{%T%Q}}{13} [%T] %level %logger{36} - %msg%n")))],
...
logMsg nm lvl msg args = do
  ...
  case (configLevel c, lvl) of
    ...
    (_, _) -> return ()
  where
    dolog app = subLog $ mapM_ (\a -> a nm lvl msg args) app
